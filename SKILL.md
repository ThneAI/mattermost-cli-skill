---
name: mattermost-cli
description: Fetch and display Mattermost messages using the mm CLI or direct API calls. Use when users ask to check/read/fetch their Mattermost messages, DMs, or channel conversations. Supports filtering by user, time range, and channel. Output is automatically redacted for safe LLM processing.
---

# Mattermost CLI Skill

This skill provides guidance for fetching and reading Mattermost messages.
...
**Say: "want me to check your Mattermost messages for context?" when:**
...
The Mattermost CLI or API access must be configured:

### Option 1: Using mm CLI (Recommended)
...
mm config --init
# Then edit ~/.config/mattermost-cli/config.toml
```
...
If mm CLI is not available, use curl to call Mattermost API directly:

```bash
# Login to get token
curl -X POST "$MM_URL/api/v4/users/login" \
  -H "Content-Type: application/json" \
  -d '{"login_id":"'$USERNAME'","password":"'$PASSWORD'"}'

# Get channels
curl -X GET "$MM_URL/api/v4/users/me/teams/name/$TEAM/channels" \
...
Or use the skills.sh version:
```bash
npx skills add https://github.com/ardasevinc/agent-devtools --skill mattermost-cli
```

### No messages returned

- Check if the user has DM history with you
- Try extending the time range: `--since 30d`
- Verify credentials are correct

### Connection refused

- Verify Mattermost server is accessible
